...
fn find_places_by_name<'a, T>(places: &'a [T], lvl: u8, name: &'a String) -> Vec<&'a T>
where
	T: AdminRegion + Sync,
{
	let mut found = Vec::new();
	for p in places.iter() {
		if p.admin_level() != lvl {
			continue;
		}
		if p.name() == name {
			found.push(p);
...
fn main() -> anyhow::Result<()> {
   // rayon::ThreadPoolBuilder::new().num_threads(std::thread::available_parallelism()?.into()).build_global().unwrap();
   ...
   let osm_src = std::env::var("OSM_SRC");
	if last == "--extract-polygons" {
		extract::extract_polygons(&osm_src.unwrap())?;
		return Ok(());
	}

	if last == "--cache" {
		extract::cache(&osm_src.unwrap())?;
		return Ok(());
	}

	if last == "--eject-via-planet" {
		return eject(eject_via_planet::load_planet_extracted_places()?, "via-planet_ultrafast_only_ru2.csv");
	}
...
pub fn fetch_places_from_pbf(input_pbf: &str) -> anyhow::Result<Vec<RawPlace>> {
	let mut pbf_reader = OsmPbfReader::new(File::open(Path::new(input_pbf))?);

	let mut places = Vec::with_capacity(1_000_000);

	'node: for el in pbf_reader.iter().map(Result::unwrap) {
		let Some(node) = el.node()
		else {
			continue;
		};
			...
			match value.as_str() {
				...
				"city" | "town" => {
					...
					for (k, v) in node.tags.iter() {
						...
						match k.as_str() {
							"name" => name = v.to_string().into(),
							"name:ru" => name_ru = v.to_string().into(),
							"addr:country" => al2 = v.to_string().into(),
							"addr:region" => al3 = v.to_string().into(),
							"addr:state" => al4 = v.to_string().into(),
							"population" => {
					...
					if let Some(name) = name {
						places.push(RawPlace { name, name_ru, al2, al4: al4.or(al3), population, lon: node.lon(), lat: node.lat() });
					}
				}
				_ => continue,
			}
...
fn select_best_parent<'a, AR: AdminRegion + Clone>(parents: &mut [&'a AR]) -> Option<&'a AR> {
	parents.sort_by(|a, b| Ord::cmp(&a.name().len(), &b.name().len()));
	parents.sort_by(|a, b| Ord::cmp(&b.tags().get(KEY_POPULATION), &a.tags().get(KEY_POPULATION)));
...
pub fn eject<AR: AdminRegion + Clone + Sync>(admin_places: Vec<AR>, output: &str) -> anyhow::Result<()> {
	...
	places.par_iter().for_each(|place| {
		...
		for admin in admin_places.iter() {
			if admin.geometry().contains(&place_location) || admin.geometry().intersects(&place_location) {
				part_of.entry(admin.admin_level()).and_modify(|admins| admins.push(admin)).or_insert(vec![admin]);
			}
		}
		...
		for (admin_level, mut regions) in part_of.into_iter() {
			...
			if let Some(admin_place) = admin_place {
				...
				if !parents_by_name.is_empty() {
					let s = parents_by_name.iter().map(|pl| pl.name().clone()).collect::<Vec<_>>().join(", ");
		...
		writeln!(
            ...
            al4 = final_parents_names.remove(&4).unwrap_or_default(),
	...
	let mut info_file = OpenOptions::new().create(true).append(true).open(output.replace(".csv", "-stats.txt"))?;

	let no_country = no_country.lock().unwrap();
	let no_country_list = no_country.iter().map(|p| format!("al4={:#?} name={:#?} name_ru={:#?}", p.al4, p.name, p.name_ru.clone().unwrap_or_default())).collect::<Vec<_>>().join("\n");
